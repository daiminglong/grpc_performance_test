...
var (
	addr       = flag.String("addr", "127.0.0.1:40021", "input server addr")
...
func handleMultiClient(addr string, totalCount, clientNum, workerNum int) {
	...
	for index := 0; index < workerNum; index++ {
		wg.Add(1)
		go func(index int) {
			var r *public.HelloReply
			var err error

			cidx := index % clientNum
			c := public.NewGreeterClient(clientPool[cidx])

			roundCount := totalCount / workerNum
			for idx := 0; idx < roundCount; idx++ {
				r, err = c.SayHello(context.Background(), &public.HelloRequest{Name: defaultName})
				if err != nil {
